describe('Dolibarr Modulo Comercial', function() {
    ...
    it('Dolibarr - Crear Pedido', function() {
        ...
        cy.get('.vmenu').find('a.vsmenu').contains('Nuevo pedido').click()
        ...
        cy.get('.border').find('input[id="s2id_autogen1"]').click({force: true}).type("s")
        cy.get('.select2-results').contains('sdgsdg dsfsdf (sdgsdg)').click()

        cy.get('.border').find('input[name="re"]').clear()
        cy.get('.border').find('button[id="reButtonNow"]').click()
        cy.get('.border').find('button[id="liv_ButtonNow"]').click()
        cy.get('.border').find('select[name="cond_reglement_id"]').select('5')
        cy.get('.border').find('select[name="mode_reglement_id"]').select('4')
        cy.get('.border').find('select[name="availability_id"]').select('2')
        cy.get('.border').find('select[name="shipping_method_id"]').select('2')
        cy.get('.border').find('select[name="demand_reason_id"]').select('3')
